...
namespace SpatialData.Converters
{
    public class DbGeographyConverter : JsonConverter
    {
        ...
        public static GeoBase ParseJsonObjectToGeoBase(JObject jsonObject, out int? coordinateSystem)
        {
            ...
            if (type == null)
            {
                throw new ArgumentException(string.Format("Expected a \"type\" property, found [{0}]",
            ...
            if (type.Type != JTokenType.String)
            {
                throw new ArgumentException(
                    string.Format("Expected a string token for the type of the GeoJSON type, got {0}", type.Type),
                    "jsonObject");
            }
        ...
        private static Tuple<byte[], int> GetWellKnownBinary(JObject jsonObject, int defaultCoordinateSystemId)
        {
            ...
            return new Tuple<byte[], int>(ob.ToArray(),
                coordinateSystemId.HasValue ? coordinateSystemId.Value : defaultCoordinateSystemId);
        }
        ...
        private static object CreateDbGeo(JObject jsonObject, Type objectType)
        {
            ...
            {
                throw new ArgumentException(
                    string.Format("Expected a DbGeography or DbGeometry objectType. Got {0}", objectType.Name),
                    "objectType");
            }

            return jsonObject.Type == JTokenType.Null || jsonObject.Type == JTokenType.None
                ? null
                : returnValue(GetWellKnownBinary(jsonObject, defaultCoordinateSystemId));
        }
        ...
        public abstract class GeoBase
        {
            ...
            public static List<List<Position>> ParseListListPosition(JArray array)
            {
                ...
                return array.Select(r => r.Select(l => new Position {P1 = (double) l[0], P2 = (double) l[1]}).ToList())
                    .ToList();
            }
            ...
            public static List<List<List<Position>>> ParseListListListPosition(JArray array)
            {
                if (array.Cast<JArray>().Any(p => p.Cast<JArray>().Any(r => r.Cast<JArray>().Any(l => l.Count < 2))))
                {
                    throw new ArgumentException(
                        string.Format(
                            "Expected all points to have greater than two points, got {0} with zero and {1} with one",
                            array.Cast<JArray>().Sum(p =>
                                p.Cast<JArray>().Sum(r => r.Cast<JArray>().Count(l => l.Count == 0))),
                            array.Cast<JArray>().Sum(p =>
                                p.Cast<JArray>().Sum(r => r.Cast<JArray>().Count(l => l.Count == 1)))),
                        "array");
                }

                return array.Select(p =>
                    p.Select(r => r.Select(l => new Position {P1 = (double) l[0], P2 = (double) l[1]}).ToList())
                        .ToList()).ToList();
            }
        ...
        public class Position : GeoBase
        {
            ...
            public override void ParseJson(JArray array)
            {
                ...
                {
                    throw new ArgumentException(
                        string.Format("Expected at least 2 points for a position, got {0}", array.Count), "array");
                }
        ...
        public class LineString : GeoBase
        {
            ...
            public override void ParseJson(JArray array)
            {
                ...
                this.Points = array.Cast<JArray>().Select(l => new Point
                    {Position = new Position {P1 = (double) l[0], P2 = (double) l[1]}}).ToList();
            }
        }
        ...
        public class Collection : GeoBase
        {
            ...
            public override void ParseJson(JArray array)
            {
                ...
                foreach (var elem in array)
                {
                    ...
                    this.Entries.Add(
                        DbGeographyConverter.ParseJsonObjectToGeoBase((JObject) elem,
                            out dummyCoordinateSystem));
                }
            }
        }
    }
